...
func AbsentOnRow(k int, sudoku [9][9]int, x int) bool {
	var y int
	for y = 0; y < 9; y++ {
		if (sudoku[x][y] == k) {
			return false
...
func AbsentOnCol(k int, sudoku [9][9]int, y int) bool {
	var x int
	for x = 0; x < 9; x++ {
		if (sudoku[x][y] == k) {
			return false;
...
func IsValid(sudoku *[9][9]int, position int) bool {
	...
	for k = 1; k <= 9; k++ {
		if (AbsentOnRow(k, *sudoku, x) && AbsentOnCol(k, *sudoku, y) && AbsentOnBloc(k, *sudoku, x, y)) {
			sudoku[x][y] = k;
